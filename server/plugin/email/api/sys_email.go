...
// @Success   200  {string}  string  "{"success":true,"data":{},"msg":"發送成功"}"
// @Router    /email/emailTest [post]
func (s *EmailApi) EmailTest(c *gin.Context) {
	err := service.ServiceGroupApp.EmailTest()
	if err != nil {
		global.GVA_LOG.Error("發送失敗!", zap.Error(err))
		response.FailWithMessage("發送失敗", c)
		return
	}
	response.OkWithMessage("發送成功", c)
}
